...
class Vspeed():
  ...
  def move(self, new_position = 0, time_secs = 2, steps = 20, easing = "LinearInOut"):
    ...
    if not self.started or new_position != self.new_position:
      ...
      self.start_time = time.ticks_ms()
      self.end_time = self.start_time + time_ms # convert to miliseconds
      self.step_delay = time_ms / steps
    ...
    #if self.new_position != self.position or self.end_time < time.ticks_ms():
    diff_time = time.ticks_ms() - self.start_time
    #print(f'diff_time: {diff_time}, time.ticks_ms: {time.ticks_ms()}, self.start_time: {self.start_time}')
